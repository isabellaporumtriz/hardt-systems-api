...
import { getStoreProducts, purchaseStoreProduct } from "@/lib/api/client-store";
...
export default function StorePage() {
  ...
  async function buyProduct(product: StoreProduct) {
    ...
    if (quantity < 1 || quantity > product.available_stock) {
      setError("Escolha uma quantidade válida.");
      return;
    }

    setBuyingId(product.id);

    let key = purchaseKeys.current[product.id];

    if (!key) {
      key = `portal-${product.id}-${quantity}-${crypto.randomUUID()}`;

      purchaseKeys.current[product.id] = key;
    }

    try {
      await purchaseStoreProduct(product.id, quantity, key);

      /*
       * Só apagamos a key após sucesso.
       */
      delete purchaseKeys.current[product.id];
      ...
      if (requestError?.response?.status === 402) {
        setError("Saldo insuficiente. Adicione saldo à sua carteira.");
    ...
    <div className="space-y-8">
      <div className="flex flex-col justify-between gap-5 lg:flex-row lg:items-end">
        <div>
          ...
          <h1 className="mt-2 text-3xl font-black text-white">Loja</h1>

          <p className="mt-2 max-w-2xl text-sm leading-6 text-white/40">
            Compre produtos disponíveis utilizando o saldo da sua carteira
            Hardt.
          </p>
        </div>
      ...
      {products.length === 0 ? (
        <div className="rounded-[28px] border border-white/[0.07] bg-white/[0.025] py-20 text-center">
          ...
          <p className="mt-2 text-sm text-white/30">
            Novos produtos aparecerão aqui quando houver estoque.
          </p>
        </div>
      ) : (
          ...
          {products.map((product) => {
            if (product.delivery_type === "service") {
              return (
                <article
                  key={product.id}
                  className="group overflow-hidden rounded-[28px] border border-violet-400/15 bg-violet-500/[0.045] transition hover:border-violet-400/30 hover:bg-violet-500/[0.07]"
                >
                  <div className="p-6">
              ...
              >
                <div className="p-6">
                  ...
                  <div className="mt-7 flex items-end justify-between">
                    <div>
                      <p className="text-xs text-white/30">Preço por unidade</p>
                    ...
                    <div className="text-right">
                      <p className="text-xs text-white/30">Estoque</p>

                      <p className="mt-1 font-bold text-emerald-300">
                        {product.available_stock} disponível
                        {product.available_stock === 1 ? "" : "is"}
                      </p>
                    </div>
                  </div>
                      ...
                      <div>
                        ...
                        <div className="mt-2 flex items-center gap-2">
                          ...
                          <button
                            ...
                            disabled={
                              buying || quantity >= product.available_stock
                            }
                            className="flex h-10 w-10 items-center justify-center rounded-xl border border-white/[0.08] bg-white/[0.04] text-white transition hover:bg-white/[0.08] disabled:cursor-not-allowed disabled:opacity-30"
                          >
                            <Plus size={16} />
